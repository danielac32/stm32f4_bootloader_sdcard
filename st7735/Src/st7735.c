...
uint16_t st7735_pinCS = 10;//PICO_DEFAULT_SPI_CSN_PIN;
uint16_t st7735_pinDC = 20;
int16_t st7735_pinRST = 16;

uint16_t st7735_pinSCK = 10;//PICO_DEFAULT_SPI_SCK_PIN;
uint16_t st7735_pinTX = 20;//PICO_DEFAULT_SPI_TX_PIN;
...
void ST7735_Reset()
{
	//if (st7735_pinRST != -1)
	//{
		hw_set_pin(GPIOx(PORT_RST3), PIN_RST3, 0);//gpio_put(st7735_pinRST, 0);
		delay(5);//sleep_ms(5);
		hw_set_pin(GPIOx(PORT_RST3), PIN_RST3, 1);//gpio_put(st7735_pinRST, 1);
	//}
}

void ST7735_Select()
{
	hw_set_pin(GPIOx(PORT_CS3), PIN_CS3, 0);//gpio_put(st7735_pinCS, 0);
}

void ST7735_DeSelect()
{
	hw_set_pin(GPIOx(PORT_CS3), PIN_CS3, 1);//gpio_put(st7735_pinCS, 1);
}

void ST7735_RegCommand()
{
	hw_set_pin(GPIOx(PORT_DC3), PIN_DC3, 0);//gpio_put(st7735_pinDC, 0);
}

void ST7735_RegData()
{
	hw_set_pin(GPIOx(PORT_DC3), PIN_DC3, 1);//gpio_put(st7735_pinDC, 1);
}
...
void ST7735_setAddrWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
	...
	xa = __bswap_constant_32(xa);
	ya = __bswap_constant_32(ya);
...
static void hal_st7735_spi_txrx16(uint16_t data){
   /*union {
        uint16_t val;
        struct {
                uint8_t lsb;
   ...
   in.val = data;
   hal_st7735_spi_txrx(in.lsb);
   hal_st7735_spi_txrx(in.msb);*/

   hal_st7735_spi_txrx((uint8_t)(data >> 8));
   hal_st7735_spi_txrx((uint8_t)data);
}
...
void LCD_WriteBitmap(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t *bitmap)
{
    ...
    for (int i = 0; i <  w * h; ++i)
    {
    	hal_st7735_spi_txrx16(bitmap[i]);
    }
