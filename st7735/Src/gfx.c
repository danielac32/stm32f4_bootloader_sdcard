...
void GFX_printf(const char *format, ...)
{
	...
	_doprnt((char *)format, args, GFX_write);
	//sprintf(printBuf,format, args);
	//_sprintf(printBuf, format, args);
	//GFX_print(printBuf);//printString(str);
    //print(printBuf);
	va_end(args);
...
{	
	if (gfxFramebuffer)
	{
	    ...
	    memcpy(gfxFramebuffer, src, 2* linesCopy);
	    memset(gfxFramebuffer+linesCopy, 0, 2* linesFill);
